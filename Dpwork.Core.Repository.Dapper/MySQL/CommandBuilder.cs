...
namespace Dpwork.Core.Repository.Dapper
{
    public class CommandBuilder : ICommandBuilder
    {
        ...
        public SqlCommand QueryCommand(string table, QueryParameter query, int? count)
        {
            ...
            if (query.Fields != null)
            {
                foreach (var column in query.Fields)
                {
                    ...
                    {
                        columnStrs.Add($"`{column.Name}` AS {column.Alias}");
                    }
                }
            }
        ...
        /// <returns></returns>
        public SqlCommand InsertCommand(string table, FieldsCollection fields, bool IsIncrement = false)
        {
            ...
            string strSql = $@"INSERT INTO `{table}`({string.Join(",", colNames)}) VALUES ({string.Join(",", prmNames)});{(IsIncrement ? "SELECT @@IDENTITY;" : "")}";
            return new SqlCommand(strSql, param);
        }
